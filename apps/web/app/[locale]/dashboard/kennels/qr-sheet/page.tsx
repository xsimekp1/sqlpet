...
export default function KennelQRSheetPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between print:hidden">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">QR kódy kotelů</h1>
          <p className="text-muted-foreground">
            Celkem: {kennels.length} kotelů
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 print:grid-cols-3 print:gap-2">
        {kennels.map((kennel) => (
          <Card key={kennel.id} className="print:border print:shadow-none">
            <CardContent className="flex flex-col items-center justify-center p-4 gap-2">
              <div className="bg-white p-2 rounded">
                <QRCode
                  value={`https://sqlpet.vercel.app/cs/public/kennels/${kennel.id}`}
                  size={120}
                  level="M"
                />
              </div>
              <div className="text-center">
                <p className="font-mono font-bold text-sm">{kennel.code}</p>
                {kennel.zone_name && (
      ...
      {kennels.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground">Žádné kotce k zobrazení</p>
          </CardContent>
        </Card>
      )}
    </div>
