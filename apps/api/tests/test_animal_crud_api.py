...
async def test_list_animals_empty(client, auth_headers, test_org_with_write_permission):
    org, _, _ = test_org_with_write_permission
    resp = await client.get(f"/orgs/{org.id}/animals", headers=auth_headers)
...
async def test_list_animals_filter_species(client, auth_headers, test_org_with_write_permission):
    org, _, _ = test_org_with_write_permission
    await client.post(f"/orgs/{org.id}/animals", json={"name": "Dog1", "species": "dog"}, headers=auth_headers)
    await client.post(f"/orgs/{org.id}/animals", json={"name": "Cat1", "species": "cat"}, headers=auth_headers)

    resp = await client.get(f"/orgs/{org.id}/animals?species=cat", headers=auth_headers)
    assert resp.status_code == 200
    body = resp.json()
    assert body["total"] == 1
...
async def test_list_animals_filter_status(client, auth_headers, test_org_with_write_permission):
    ...
    resp = await client.get(f"/orgs/{org.id}/animals?status=available", headers=auth_headers)
    assert resp.status_code == 200
    body = resp.json()
    assert body["total"] == 1
...
async def test_list_animals_search(client, auth_headers, test_org_with_write_permission):
    org, _, _ = test_org_with_write_permission
    await client.post(f"/orgs/{org.id}/animals", json={"name": "Maxík", "species": "dog"}, headers=auth_headers)
    await client.post(f"/orgs/{org.id}/animals", json={"name": "Bella", "species": "cat"}, headers=auth_headers)

    resp = await client.get(f"/orgs/{org.id}/animals?search=max", headers=auth_headers)
    assert resp.status_code == 200
    body = resp.json()
    assert body["total"] == 1
...
async def test_list_animals_pagination(client, auth_headers, test_org_with_write_permission):
    ...
    resp = await client.get(f"/orgs/{org.id}/animals?page=1&page_size=2", headers=auth_headers)
    assert resp.status_code == 200
    body = resp.json()
    assert body["total"] == 5
...
async def test_get_animal(client, auth_headers, test_org_with_write_permission):
    ...
    resp = await client.get(f"/orgs/{org.id}/animals/{animal_id}", headers=auth_headers)
    assert resp.status_code == 200
...
async def test_get_animal_not_found(client, auth_headers, test_org_with_write_permission):
    org, _, _ = test_org_with_write_permission
    fake_id = uuid.uuid4()
    resp = await client.get(f"/orgs/{org.id}/animals/{fake_id}", headers=auth_headers)
...
async def test_update_animal(client, auth_headers, test_org_with_write_permission):
    ...
    resp = await client.patch(
        f"/orgs/{org.id}/animals/{animal_id}",
        json={"name": "NewName"},
        headers=auth_headers,
    )
    assert resp.status_code == 200
...
async def test_delete_animal(client, auth_headers, test_org_with_write_permission):
    ...
    resp = await client.delete(f"/orgs/{org.id}/animals/{animal_id}", headers=auth_headers)
...
async def test_delete_animal_soft(client, auth_headers, test_org_with_write_permission, db_session):
    ...
    await client.delete(f"/orgs/{org.id}/animals/{animal_id}", headers=auth_headers)
