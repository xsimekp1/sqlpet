...
def calculate_mer(
    weight_kg: float,
    species: str,               # dog / cat / rabbit / bird / other
    ...
    if species == "cat":
        activity_entry = _ACTIVITY_CAT.get(activity_key) or _ACTIVITY_CAT.get(f"unknown_{age_group}") or (1.2, "Kočka")
    else:
        # dog and all other species use dog table as default
        activity_entry = _ACTIVITY_DOG.get(activity_key) or _ACTIVITY_DOG.get(f"unknown_{age_group}") or (1.4, "Pes")
    ...
    # --- Health modifier ---
    health_val, health_label = HEALTH_FACTORS.get(health_modifier, HEALTH_FACTORS["healthy"])

    # --- Environment ---
    env_val, env_label = ENVIRONMENT_FACTORS.get(environment, ENVIRONMENT_FACTORS["indoor"])

    # --- Breed size ---
    size_val, size_label = BREED_SIZE_FACTORS.get(breed_size, BREED_SIZE_FACTORS["unknown"])

    # --- Weight goal ---
    goal_val, goal_label = WEIGHT_GOAL_FACTORS.get(weight_goal, WEIGHT_GOAL_FACTORS["maintain"])
